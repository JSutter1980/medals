...
function App() {
  ...
  async function handleLogin(username, password) {
    // local user1 account with all roles
    if (username === "user1" && password === "password1") {
      const header = btoa(JSON.stringify({ alg: "HS256", typ: "JWT" }));
      const payload = btoa(JSON.stringify({
        username: "user1",
        roles: ["medals-post", "medals-patch", "medals-delete"],
        exp: Math.floor(Date.now() / 1000) + 60 * 60 * 24,
      }));
      const fakeToken = `${header}.${payload}.local`;
  ...
  function getSortedCountries() {
    return [...countries].sort((a, b) => {
      ...
      if (sortBy === "total") {
        const totalA = medals.current.reduce((s, m) => s + a[m.name].page_value, 0);
        const totalB = medals.current.reduce((s, m) => s + b[m.name].page_value, 0);
  ...
  return (
    <Theme appearance={appearance}>
      ...
      <Flex p="2" pl="8" className="fixedHeader" justify="between" align="center" wrap="wrap" gap="2">
        <Heading size="6">
          Olympic Medals
        ...
        <SegmentedControl.Root value={sortBy} onValueChange={setSortBy} size="1">
          <SegmentedControl.Item value="name">Name</SegmentedControl.Item>
          <SegmentedControl.Item value="total">Total</SegmentedControl.Item>
          <SegmentedControl.Item value="gold">Gold</SegmentedControl.Item>
          <SegmentedControl.Item value="silver">Silver</SegmentedControl.Item>
          <SegmentedControl.Item value="bronze">Bronze</SegmentedControl.Item>
